...
export class KrakenFuturesClient {
  ...
  private async getSpot<T>(path: string, params?: Record<string, any>): Promise<T> {
    const qs = new URLSearchParams();
    if (params) for (const [k, v] of Object.entries(params)) if (v !== undefined && v !== null) qs.set(k, String(v));
    const url = `${this.spotProxy}?path=${encodeURIComponent(path)}&qs=${encodeURIComponent(qs.toString())}`;
    const r = await this.fetcher(url, { headers: { 'Accept': 'application/json' } });
    if (!r.ok) { const t = await r.text().catch(()=> ''); throw new Error(`SPOT HTTP ${r.status} ${r.statusText} – ${t}`); }
    return r.json() as Promise<T>;
  }
  ...
  async getTrades(symbol: string, opts: { since?: number; until?: number } = {}): Promise<KrakenTrade[]> {
    const res: any = await this.get('/derivatives/api/v3/trades', {
      symbol, since: opts.since, until: opts.until, count: 1000
    });
  ...
  async getSpotOhlc(pair: string, intervalMin: number, since?: number): Promise<Ohlc[]> {
    // Kraken Spot OHLC returns arrays: [time, open, high, low, close, vwap, volume, count]
    const res: any = await this.getSpot('/0/public/OHLC', {
      pair, interval: intervalMin, since: since ? Math.floor(since/1000) : undefined
    });
    const firstKey = Object.keys(res?.result || {}).find(k => k !== 'last');
    const rows: any[] = firstKey ? res.result[firstKey] || [] : [];
  ...
  async getOhlc(symbol: string, interval: string, opts: { since?: number; until?: number } = {}): Promise<Ohlc[]> {
    ...
    // 2) Fallback: Futures trades -> 1m aggregation
    try {
      ...
      if (trades.length > 0) {
        ...
        return Array.from(bucket.entries()).sort((a,b)=>a[0]-b[0]).map(([t,b]) => ({
          time: t, open: b.o, high: b.h, low: b.l, close: b.c, volume: b.v
        }));
      }
    } catch (e) {
      // swallow and try spot
    }

    // 3) Final fallback: Kraken Spot OHLC for XBTUSD, 1-minute
    return this.getSpotOhlc('XBTUSD', 1, opts.since);
  }
}
