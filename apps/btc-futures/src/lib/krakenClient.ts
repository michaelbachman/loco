...
export class KrakenFuturesClient {
  private spotProxy: string = '/.netlify/functions/krakenSpot'
...
async getOhlc(symbol: string, interval: string, opts: { since?: number; until?: number } = {}): Promise<Ohlc[]> {
  ...
  // Fallback: Futures Trades aggregation
  try {
  ...
  // Final fallback: Kraken Spot OHLC (XBTUSD), 1-minute
  const spot = await this.getSpotOhlc('XBTUSD', 1, opts.since)
  return spot
}
